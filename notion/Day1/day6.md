# Day 6: Status UI for Long-Running Task

# 0. 목표

<aside>
💡 **오래 걸리는 작업의 진행 상황을 시각적으로 보여주는 상태 표시줄(Status UI) 구현**

1. LLM 호출처럼 시간이 걸리는 작업에서 사용자 경험(UX)을 개선합니다.
2. `st.status` 컨테이너를 사용하여 작업 단계를 상세히 보여줍니다.
3. 완료 후 상태창을 접어 결과를 강조하는 패턴을 익힙니다.

</aside>

# 1. 개념 및 이론 (Theory)

### UX와 피드백 (Transparency)
버튼을 눌렀는데 3초 동안 아무 반응이 없으면 사용자는 "고장 났나?"라고 생각하여 버튼을 여러 번 누르거나 창을 닫아버립니다. **"지금 ~를 하고 있습니다"** 라고 알려주는 것만으로도 사용자는 훨씬 더 오래 기다려줄 수 있습니다.

### st.spinner vs st.status
- **`st.spinner`**: 단순하게 뱅글뱅글 도는 UI. "기다리세요"라는 메시지만 전달합니다.
- **`st.status`**: "문서 분석 중...", "초안 작성 중...", "검수 중..." 처럼 복잡한 단계를 리스트 형태로 보여주고, 완료되면 로그처럼 남겨둘 수 있는 고급 UI입니다.

# 2. 단계별 구현 (Step-by-Step)

Day 5의 코드를 업그레이드하여 `day6.py`를 만듭니다.

### Step 1: 앱 구조 복사 (Day 5 기반)

```python
import streamlit as st
import time
from snowflake.cortex import Complete
# 연결 코드 생략...

st.title("Post Generator with Status 🟢")
# ... 위젯 코드(url, tone 등)는 Day 5와 동일 ...
```

### Step 2: st.status 적용

`st.button` 내부 로직을 다음과 같이 변경합니다.

```python
if st.button("Generate Post"):
    
    # st.status를 사용하여 진행 상태창을 엽니다. expanded=True는 처음부터 펼쳐서 보여준다는 뜻입니다.
    with st.status("Initializing agent...", expanded=True) as status:
        
        # 단계 1: 준비
        st.write("🔍 Reading configuration...")
        time.sleep(1) # 실제 작업인 척 시간을 끕니다.
        
        # 단계 2: 프롬프트 작성
        st.write("✏️ Drafting prompt...")
        prompt = f"Create a {tone} post about {url}..."
        time.sleep(1)
        
        # 단계 3: LLM 호출
        st.write("🤖 Contacting Snowflake Cortex AI...")
        response = Complete("claude-3-5-sonnet", prompt, session=session)
        
        # 단계 4: 완료 처리
        # update() 함수로 상태창의 제목과 상태(complete)를 변경하고 창을 접습니다(expanded=False).
        status.update(label="Post generated successfully!", state="complete", expanded=False)
        
    # 상태창 밖에서 결과를 보여줍니다.
    st.subheader("Final Output")
    st.markdown(response)
```

# 3. 핵심 포인트 (Key Takeaways)

- **사용자 심리**: 진행 상황이 구체적일수록 사용자는 체감 대기 시간을 짧게 느낍니다.
- **`status.update`**: 작업이 끝나면 "성공!"으로 라벨을 바꾸고 창을 닫아줌으로써, 사용자의 시선이 자연스럽게 결과물(Output)로 이동하도록 유도하는 디자인 패턴입니다.

---

# 💡 실습 과제 (Hands-on Practice)

사용자에게 작업의 진행 단계를 상세히 보여주는 상태창을 구현해 봅니다.

1. `with st.status("엔진 구동 중...") as status:` 구문을 사용하여 상태 컨테이너를 만드세요.
2. `st.write()`를 사용하여 최소 2개 이상의 중간 단계(예: "프롬프트 작성 중...", "Cortex 호출 중...")를 표시하세요.
3. 모든 작업이 완료되면 `status.update()`를 사용하여 라벨을 "완료되었습니다!"로 바꾸고 상태를 `complete`로 변경하세요.

# ✅ 정답 코드 (Solution)

```python
# st.status를 이용한 상태 UI 구현
with st.status("엔진 구동 중...", expanded=True) as status:
    # 1. 중간 단계 표시
    st.write("🔍 데이터를 분석하고 프롬프트를 구성합니다.")
    prompt = f"Create a {tone} LinkedIn post about {content} in {word_count} words."
    time.sleep(1) # 지연 효과
    
    # 2. 메인 작업 실행
    st.write("🤖 Snowflake Cortex API를 호출합니다.")
    response = call_cortex_llm(prompt)
    
    # 3. 상태 업데이트 및 완료
    status.update(label="✅ 게시물 생성 완료!", state="complete", expanded=False)
```
